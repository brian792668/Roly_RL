...
class Robot_system:
    ...
    def thread_RL_move(self):
        while not self.stop_event.is_set():
            ...
            state = np.concatenate([guide_xyz.copy(), guidetohand_norm.copy(), action_old[0:3], joints[2:4], joints[5:7], [joints[5]], [0]]).astype(np.float32)
    ...
    def reachable(self, point):
        ...
        if dis2shoulder >= 0.55 or dis2shoulder <= 0.39:
            return False
        else:
            return True
