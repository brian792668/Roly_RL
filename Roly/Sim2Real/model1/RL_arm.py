...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            self.motor.pos_ctrl = self.motor.toRolyctrl([ 0,
                                                          0,
                                                          np.degrees(self.sys.ctrlpos[3]), 
                                                          np.degrees(self.sys.ctrlpos[4]), 
                                                          np.degrees(self.sys.ctrlpos[5]),
                                                          -np.degrees(self.sys.ctrlpos[6]), 
                                                          -np.degrees(self.sys.ctrlpos[7]), 
                                                          0])
            self.motor.writeAllMotorPosition(self.motor.pos_ctrl)
    ...
    def get_reward(self):
        ...
        r1 = 30*(self.sys.hand2target - new_dis)
        if r1 >= 0: r1 = 0
