...
class Robot_system:
    def __init__(self):
        ...
        # Initial RL policy
        RL_path1 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "RLmodel/model_1/v24_with_hand_length/model.zip")
        ...
        self.DH_table_R = DHtable([[    0.0, np.pi/2,   -0.028,  -0.104],
                                   ...
                                   [    0.0, np.pi/2,     0.0,     0.0],
                                   [    0.0, np.pi/2,     0.0,  0.22+0.05]]) # 0.1803
    ...
    def thread_RL_move(self):
        while not self.stop_event.is_set():
            ...
            state = np.concatenate([guide_xyz.copy(), guidetohand_norm.copy(), action_old[0:3], joints[2:4], joints[5:7], [0.05]]).astype(np.float32)
