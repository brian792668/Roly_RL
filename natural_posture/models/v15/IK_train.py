...
def train(numberofpoints, version):
    ...
    # ---------------- Create datasets -----------------
    xyz_array = np.load(os.path.join(file_path, f"../../datasets/new_origin/new_{numberofpoints}points/xyz.npy"))
    joints_array = np.load(os.path.join(file_path, f"../../datasets/new_origin/new_{numberofpoints}points/joints.npy"))
    ik_dataset = NPDataset(xyz_array, joints_array)
    ...
    test_dataset = NPDataset(test_xyz_array, test_joints_array)
...
if __name__ == '__main__':
    train(numberofpoints=9876, version="v15")
