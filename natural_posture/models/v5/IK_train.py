...
def train(numberofpoints, version):
    # ---------------- Create datasets -----------------
    xyz_array = np.load(f'Roly/Inverse_kinematics/datasets/{numberofpoints}points2/xyz.npy')
    joints_array = np.load(f'Roly/Inverse_kinematics/datasets/{numberofpoints}points2/joints.npy')
    ik_dataset = IKDataset(xyz_array, joints_array)
    dataloader = DataLoader(ik_dataset, batch_size=20, shuffle=True)
...
if __name__ == '__main__':
    train(numberofpoints=2000, version="v5")
