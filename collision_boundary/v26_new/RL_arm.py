...
class RL_arm(gym.Env):
    def __init__(self):
        ...
        self.CBmodel.eval()
        self.IK = IKMLP()
        # self.IK.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), "MLP_IK.pth"), weights_only=True))
        self.IK.load_state_dict(torch.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), "IKMLP_full.pth"), weights_only=True))
        self.IK.eval()
    ...
    def step(self, action): 
        if self.viewer.is_running() == False:
            ...
            # # input_tensor = torch.tensor(np.concatenate([self.sys.vec_guide2neck.copy(), [0], [self.sys.guide_arm_joints[3]]]).astype(np.float32))
            # input_tensor = torch.tensor(np.concatenate([self.sys.vec_guide2neck.copy(), [0], [self.sys.pos[5]]]).astype(np.float32))
            # with torch.no_grad():
            #     IKoutput = self.IK(input_tensor)
            # output = IKoutput.tolist()
            # for i in range(int(1.0/self.sys.Hz/0.005)):
            #     self.sys.ctrlpos[2] = self.sys.ctrlpos[2] + np.tanh(2*(1.25*output[0] - 1.25*self.sys.ctrlpos[2]))*0.01
            #     self.sys.ctrlpos[3] = self.sys.ctrlpos[3] + np.tanh(2*(1.25*output[1] - 1.25*self.sys.ctrlpos[3]))*0.01
            #     self.sys.ctrlpos[4] = 0
            #     self.sys.ctrlpos[5] = self.sys.ctrlpos[5] + np.tanh(2*(self.sys.guide_arm_joints[3]- self.sys.ctrlpos[5]))*0.01
            #     self.sys.ctrlpos[6] = self.sys.ctrlpos[6] + np.tanh(2*(1.25*output[2] - 1.25*self.sys.ctrlpos[6]))*0.01
            #     self.sys.ctrlpos[7] = self.sys.ctrlpos[7] + self.sys.joints_increment[4]*0.01
            #     self.control_and_step()
            self.render()
    ...
    def print_scale(self, low, high, elbow_now, elbow_input, collision, reward):
        ...
        if collision <= 0.5 and start+5<pos_c<end-5:
            output[pos_c] = f' \033[91m{elbow_now:.0f}\033[0m '
        elif start+5<pos_c<end-5:
