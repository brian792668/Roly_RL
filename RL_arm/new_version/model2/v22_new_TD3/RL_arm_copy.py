...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        if self.viewer.is_running() == False:
            ...
            self.sys.pos_guide[0] = self.sys.pos_target[0] + 0.08*self.inf.action[0]
            self.sys.pos_guide[1] = self.sys.pos_target[1] + 0.08*self.inf.action[1]
            self.sys.pos_guide[2] = self.sys.pos_target[2] + 0.08*self.inf.action[2]
            self.get_reward()
