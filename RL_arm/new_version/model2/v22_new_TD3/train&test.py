...
def test(model, env, model_path, render_speed=0):
    env.render_speed = render_speed
    model = stable_baselines3.TD3.load(model_path, env)
    obs, _ = env.reset()
    env.inf.totaltimestep = 0
    env.inf.total_reward = 0
...
if __name__ == '__main__':
    ...
    if os.path.exists(model_last_path):
        RL_model = stable_baselines3.TD3.load(model_last_path, my_env)
    else:
        RL_model = stable_baselines3.TD3('MlpPolicy', my_env, learning_rate=0.0010)
