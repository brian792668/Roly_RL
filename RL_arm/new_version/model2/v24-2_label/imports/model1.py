...
class RLmodel:
    def __init__(self):
        ...
        self.model = SAC.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), "current_model.zip"))
    ...
    def predict(self):
        # self.state = np.concatenate([self.obs_guide_to_neck.copy(), self.obs_guide_to_hand_norm.copy(), self.action.copy(), self.obs_joints.copy(), [self.obs_hand_dis]]).astype(np.float32)
