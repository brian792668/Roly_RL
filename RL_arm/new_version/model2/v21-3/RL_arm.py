...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        if self.viewer.is_running() == False:
            ...
            self.observation_space = np.concatenate([self.sys.vec_target02neck, self.sys.vec_hand2neck, self.sys.vec_hand2elbow, self.obs.joint_arm]).astype(np.float32)
    ...
    def spawn_new_point(self):
        if self.inf.timestep == 0 or self.sys.hand2target <= 0.03:
            reachable = False
