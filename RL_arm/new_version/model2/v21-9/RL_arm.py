...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        if self.viewer.is_running() == False:
            ...
            new_guide = self.sys.pos_hand.copy()
            new_guide[0] += self.inf.action[0]*0.1
            new_guide[1] += self.inf.action[1]*0.1
            new_guide[2] += self.inf.action[2]*0.1
            ...
            self.observation_space = np.concatenate([self.sys.vec_target2neck, 
                                                     self.sys.vec_target2hand,
                                                     self.obs.joint_arm[0:4],
    ...
    def get_reward(self):
        ...
        new_dis = ( (self.sys.pos_target[0]-self.sys.pos_guide[0])**2 + (self.sys.pos_target[1]-self.sys.pos_guide[1])**2 + (self.sys.pos_target[2]-self.sys.pos_guide[2])**2 ) **0.5
        r2 = np.exp(-50*new_dis**2)
        r4 = np.exp(-2000*new_dis**4)
        ...
        self.inf.reward += 0.3*r2*r1 + 0.7*r3 + r4
        # print(f"reward: {self.inf.reward:.2f}")
    ...
    def compensate(self):
        ...
        new_guide[0] = self.sys.pos_target[0] - self.sys.grasping_dis*np.cos(np.pi*self.inf.action[1])*np.cos(np.pi*self.inf.action[0])
        new_guide[1] = self.sys.pos_target[1] - self.sys.grasping_dis*np.cos(np.pi*self.inf.action[1])*np.sin(np.pi*self.inf.action[0])
        new_guide[2] = self.sys.pos_target[2] + self.sys.grasping_dis*np.sin(np.pi*self.inf.action[1])
        if self.check_reachable(point=new_guide) == True:
            self.sys.pos_guide = new_guide.copy()
            self.sys.vec_guide2hand  = [self.sys.pos_guide[0] - self.sys.pos_hand[0] , self.sys.pos_guide[1] - self.sys.pos_hand[1] , self.sys.pos_guide[2] - self.sys.pos_hand[2]]
