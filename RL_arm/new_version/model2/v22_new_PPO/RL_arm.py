...
class RL_arm(gym.Env):
    ...
    def get_state(self):
        ...
        self.sys.pos_grasp_point[0] = self.sys.pos_temp_target[0] - 0.08*np.cos(np.pi*self.inf.action[1])*np.cos(np.pi*self.inf.action[0])
        self.sys.pos_grasp_point[1] = self.sys.pos_temp_target[1] - 0.08*np.cos(np.pi*self.inf.action[1])*np.sin(np.pi*self.inf.action[0])
        self.sys.pos_grasp_point[2] = self.sys.pos_temp_target[2] + 0.08*np.sin(np.pi*self.inf.action[1])   
