...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        if self.viewer.is_running() == False:
            ...
            self.sys.joints_increment[4] = self.inf.action[3]
            ...
            self.observation_space = np.concatenate([self.sys.vec_target02neck, 
                                                     self.sys.vec_hand2neck, 
                                                     self.sys.vec_hand2elbow, 
                                                     self.inf.action, 
    ...
    def get_reward(self):
        ...
        self.inf.reward += 0.5*r2*reachable + 0.5*r1*r3
        # self.inf.reward += ( ratio*r1*r3 + (1-ratio)*r2 )*reachable
        # up = int(255*ratio)
        # low = int(255*(1-ratio))
    ...
    def get_state(self):
        ...
        self.obs.joint_arm[2:5] = self.data.qpos[12:15].copy()
    ...
    def spawn_new_point(self):
        if self.inf.timestep == 0 or self.sys.hand2target <= 0.05:
            ...
            while reachable == False:
                ...
                reachable = self.check_reachable(self.sys.pos_target0.copy())
    ...
    def compensate(self):
        # self.sys.pos_hand   = self.data.site_xpos[mujoco.mj_name2id(self.robot, mujoco.mjtObj.mjOBJ_SITE, f"R_hand_marker")].copy()
        # self.sys.pos_target = [self.sys.pos_hand[0]*0.7 + self.sys.pos_target0[0]*0.3,  self.sys.pos_hand[1]*0.7 + self.sys.pos_target0[1]*0.3,  self.sys.pos_hand[2]*0.7 + self.sys.pos_target0[2]*0.3]
        # self.sys.vec_target2hand  = [self.sys.pos_target[0] - self.sys.pos_hand[0] , self.sys.pos_target[1] - self.sys.pos_hand[1] , self.sys.pos_target[2] - self.sys.pos_hand[2]]
        # self.sys.hand2target = ( self.sys.vec_target2hand[0]**2 + self.sys.vec_target2hand[1]**2 + self.sys.vec_target2hand[2]**2 ) ** 0.5
        ...
        self.sys.pos_hand = self.data.site_xpos[mujoco.mj_name2id(self.robot, mujoco.mjtObj.mjOBJ_SITE, f"R_hand_marker")].copy()
        self.sys.pos_target[0] = self.sys.pos_target0[0] - self.sys.grasping_dis*np.cos(np.pi*self.inf.action[1])*np.cos(np.pi*self.inf.action[0])
        self.sys.pos_target[1] = self.sys.pos_target0[1] - self.sys.grasping_dis*np.cos(np.pi*self.inf.action[1])*np.sin(np.pi*self.inf.action[0])
        self.sys.pos_target[2] = self.sys.pos_target0[2] + self.sys.grasping_dis*np.sin(np.pi*self.inf.action[1])
        self.sys.vec_target2hand  = [self.sys.pos_target[0] - self.sys.pos_hand[0] , self.sys.pos_target[1] - self.sys.pos_hand[1] , self.sys.pos_target[2] - self.sys.pos_hand[2]]
