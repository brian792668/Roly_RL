...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            for i in range(len(action)):
                ...
                self.inf.action_new[i] += action[i]*0.05
                if self.inf.action_new[i]>=1: self.inf.action_new[i]=1
                if self.inf.action_new[i]<=-1: self.inf.action_new[i]=-1

            for i in range(int(1/self.sys.Hz/0.005)):
                self.sys.ctrlpos[2] = self.sys.ctrlpos[2] + self.inf.action_new[0]*0.01
                self.sys.ctrlpos[3] = self.sys.ctrlpos[3] + self.inf.action_new[1]*0.01
                self.sys.ctrlpos[4] = 0
                self.sys.ctrlpos[5] = self.sys.ctrlpos[5] + np.tanh(self.sys.arm_target_pos[3] - self.sys.pos[5])*0.01
                self.sys.ctrlpos[6] = self.sys.ctrlpos[6] + self.inf.action_new[2]*0.01
    ...
    def get_reward(self):
        ...
        self.inf.total_reward_standard += (0.8*r0 + r2)*r1
    ...
    def spawn_new_point(self):
        ...
        if self.inf.timestep == 0 or self.sys.hand2target <= 0.05 or hand_camera_center <= 5:
            ...
            while reachable == False:
                self.sys.pos_target0[0] = random.uniform(-0.05, 0.70)
                self.sys.pos_target0[1] = random.uniform(-0.95, 0.45)
                self.sys.pos_target0[2] = random.uniform( 0.60, 1.40)
