...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            alpha1 = 1-0.95*np.exp(-300*self.sys.hand2target**2)
            # alpha2 = 0
            ...
            for i in range(int(1/self.sys.Hz/0.001)):
                self.sys.ctrlpos[0] = self.sys.ctrlpos[0] + self.inf.action[0]*0.002*alpha1
                self.sys.ctrlpos[1] = self.sys.ctrlpos[1] + self.inf.action[1]*0.002*alpha1
                self.sys.ctrlpos[2] = 0
                self.sys.ctrlpos[3] = self.sys.ctrlpos[3] + np.tanh(self.sys.arm_target_pos[3] - self.sys.pos[3])*0.002
                self.sys.ctrlpos[4] = self.sys.ctrlpos[4] + self.inf.action[2]*0.002*alpha1
            ...
            self.get_state()
            self.observation_space = np.concatenate([self.obs.obj_to_neck_xyz, self.obs.obj_to_hand_xyz_norm, self.obs.joint_arm]).astype(np.float32)
            self.inf.truncated = False
            self.inf.info = {}
            return self.observation_space, self.inf.reward, self.inf.done, self.inf.truncated, self.inf.info
