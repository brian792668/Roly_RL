...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            for i in range(int(1/self.sys.Hz/0.001)):
                self.sys.ctrlpos[3] = self.sys.ctrlpos[3] + self.inf.action[0]*0.002
                self.sys.ctrlpos[4] = self.sys.ctrlpos[4] + np.tanh(self.sys.arm_target_pos[1] - self.sys.pos[4])*0.002
                self.sys.ctrlpos[5] = 0
                self.sys.ctrlpos[6] = self.sys.ctrlpos[6] + self.inf.action[1]*0.002
                self.sys.ctrlpos[7] = self.sys.ctrlpos[7] + self.inf.action[2]*0.002
    ...
    def get_reward(self):
        ...
        new_dis = (self.data.qpos[16]-self.sys.pos_hand[0])**2 + (self.data.qpos[17]-self.sys.pos_hand[1])**2 + (self.data.qpos[18]-self.sys.pos_hand[2])**2
        new_dis = new_dis ** 0.5
