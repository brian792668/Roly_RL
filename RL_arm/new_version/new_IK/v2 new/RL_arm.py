...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            input_tensor = torch.tensor(np.concatenate([self.obs.obj_to_neck_xyz.copy(), [self.sys.arm_target_pos[3]]]).astype(np.float32))
            with torch.no_grad():
            ...
            for i in range(int(1/self.sys.Hz/0.005)):
                ...
                self.sys.ctrlpos[2] = self.sys.ctrlpos[2] + np.tanh(1.25*output[0] - 1.25*self.sys.pos[2])*0.01
                self.sys.ctrlpos[3] = self.sys.ctrlpos[3] + np.tanh(1.25*output[1] - 1.25*self.sys.pos[3])*0.01
                self.sys.ctrlpos[4] = 0
                self.sys.ctrlpos[5] = self.sys.ctrlpos[5] + np.tanh(0.5*self.sys.arm_target_pos[3] - 0.5*self.sys.pos[5])*0.01
                self.sys.ctrlpos[6] = self.sys.ctrlpos[6] + np.tanh(1.25*output[2] - 1.25*self.sys.pos[6])*0.01
