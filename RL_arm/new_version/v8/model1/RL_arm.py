...
class RL_arm(gym.Env):
    ...
    def step(self, action): 
        ...
        if self.viewer.is_running() == False:
            ...
            alpha1 = 1-0.9*np.exp(-300*self.sys.hand2target**2)
            alpha2 = 0
            ...
            alpha2 = 1-0.9*np.exp(-50*alpha2**2)
    ...
    def reset(self, seed=None, **kwargs): 
        if self.viewer.is_running() == False:
            ...
            self.sys.random_arm_pos = [ -dummy_random,
                                        ...
                                        0.0 ]
            for i in range(100):
                self.sys.ctrlpos[3] = self.sys.pos[3] + 0.002*np.tanh(self.sys.random_arm_pos[0] - self.sys.pos[3])
                self.sys.ctrlpos[4] = self.sys.pos[4] + 0.002*np.tanh(self.sys.random_arm_pos[1] - self.sys.pos[4])
                self.sys.ctrlpos[5] = self.sys.pos[5] + 0.002*np.tanh(self.sys.random_arm_pos[2] - self.sys.pos[5])
                self.sys.ctrlpos[6] = self.sys.pos[6] + 0.002*np.tanh(self.sys.random_arm_pos[3] - self.sys.pos[6])
                self.sys.ctrlpos[7] = self.sys.pos[7] + 0.002*np.tanh(self.sys.random_arm_pos[4] - self.sys.pos[7])
                self.sys.ctrlpos[8] = self.sys.pos[8] + 0.002*np.tanh(self.sys.random_arm_pos[5] - self.sys.pos[8])
                self.sys.pos = [self.data.qpos[i] for i in controlList]
                self.sys.vel = [self.data.qvel[i-1] for i in controlList]
                self.data.ctrl[:] = self.sys.PIDctrl.getSignal(self.sys.pos, self.sys.vel, self.sys.ctrlpos)
                mujoco.mj_step(self.robot, self.data)
