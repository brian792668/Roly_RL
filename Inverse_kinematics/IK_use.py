...
class IKMLP(nn.Module):
    def __init__(self):
        ...
        self.fc2 = nn.Linear(64, 64) # 隱藏層64輸出128
        self.fc3 = nn.Linear(64, 64) # 隱藏層64輸出128
        self.fc4 = nn.Linear(64, 4)  # 輸出4維（4個關節角度）
    
    def forward(self, x):
        x = F.relu(self.fc1(x))  # 第1層用 ReLU
        x = F.relu(self.fc2(x))  # 第2層用 ReLU
        x = F.relu(self.fc3(x))  # 第3層用 ReLU
        x = self.fc4(x)          # 最後一層直接輸出
        return x

model = IKMLP()
model.load_state_dict(torch.load('Roly/Inverse_kinematics/models/v5/IKmodel_v5.pth'))
