...
def lebal_Roly_IK(numberofpoints = 2000):
    ...
    # ================================= 2000 points ======================================
    def check(point):
        dtoshoulder = ( (point[0]-0.0)**2 + (point[1]+0.2489)**2 + (point[2]-1.34)**2 ) **0.5
        if dtoshoulder >= 0.48 or dtoshoulder <= 0.27:
            return False
    ...
    while viewer.is_running():
        ...
        targetpoint[0] += 0.0001*np.tanh(100*(0.98*sorted_targetpoints[point_index+numberofdelete][0]-0.98*targetpoint[0]))
        targetpoint[1] += 0.0001*np.tanh(100*(0.98*sorted_targetpoints[point_index+numberofdelete][1]-0.98*targetpoint[1]))
        targetpoint[2] += 0.0001*np.tanh(100*(0.98*sorted_targetpoints[point_index+numberofdelete][2]-0.98*targetpoint[2]))
        ...
        # data.xfrc_applied[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "R_arm2")][0] = -2
